...
- [x] 移动端 `apps/mobile/` 可启动并打开页面（H5 或小程序其一即可）
- [x] 管理端 `apps/admin/` 可启动并打开页面（merchant/admin 均可登录）
...
- [x] 登录 merchant 后，仅展示商户菜单与页面（路由不可越权）
- [x] 酒店录入/编辑页：必做维度齐全并校验（缺字段有明确提示）
  - [x] 酒店名（中/英）、地址、星级、房型、价格、开业时间
- [x] 房型管理：可新增/删除/修改房型，价格校验合理（≥ 0）
- [x] 商户酒店列表：展示 `auditStatus/onlineStatus`，支持编辑与提交审核
- [x] 被拒绝酒店：能看到 `rejectReason`，修改后可再次提交审核

---

## 3. 管理端（C）阶段3：管理员端（admin）页面验收

- [x] 登录 admin 后，仅展示管理员菜单与页面（路由不可越权）
- [x] 审核列表：可筛选 `pending/approved/rejected` + `online/offline`，分页可用
- [x] 审核拒绝：必须填写原因；列表/详情能展示原因
- [x] 发布上线：仅允许 `approved` 酒店发布；发布后用户端可见
- [x] 下线：下线后用户端不可见；可恢复上线（再次发布）

---

## 4. 移动端（B）阶段3：体验与性能验收（最终阶段）

- [x] 列表页上拉加载稳定（不重复请求、不乱序），loading/empty/error 状态完整
- [x] 详情页房型按价格从低到高展示（与商户录入价格一致）
- [x] 交互优化：按钮防连点、请求失败提示友好、返回后条件保留（至少做到 1–2 个亮点）
- [x] 长列表性能：至少有“明显措施”（节流/分页渲染/虚拟列表其一）并能说明收益
