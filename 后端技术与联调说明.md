...
- 地理编码：`GET /api/v1/map/geocode`
  - 参数：`address`（完整地址）
  - 返回：百度地理编码标准响应（`status=0` 表示成功）
- 静态地图：`GET /api/v1/map/staticimage`
  - 参数：`lng`、`lat`、`width`、`height`、`zoom`、`markers`
  - 返回：`image/png`

环境变量：
- `server/.env` 中配置 `BAIDU_AK`（服务端 AK）
