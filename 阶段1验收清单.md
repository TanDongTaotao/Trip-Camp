...
- [x] 注册 user：请求体包含 `role=user`，响应包含 `token`，且 `user.role=user`
...
- [x] 登录 user：响应包含 `token`，且 `user.role=user`
...
- [x] user token：返回 `200` 且 `user.role=user`
...
- [x] Login 页调用 `POST /auth/login` 成功后将 token 写入本地存储（storage）
- [x] Home 页调用 `GET /auth/me`：
  - [x] 已登录：能展示/Toast 出用户名或 role（任一即可）
  - [x] 未登录/过期：跳转到 Login
- [x] 请求封装自动携带 `Authorization`，且非 2xx 能提示错误并抛出（避免静默失败）
